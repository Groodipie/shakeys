...
$nav = [
  'dashboard' => ['label' => 'Dashboard', 'icon' => 'bi-speedometer2', 'href' => '/staff/dashboard'],
  'orders'    => ['label' => 'Orders',    'icon' => 'bi-receipt',      'href' => '/staff/orders'],
];
?>
<!DOCTYPE html>
<html lang="en">
...
<title><?= e($pageTitle ?? "Staff — Shakey's") ?></title>
...
.staff-shell{flex:1;display:flex;align-items:stretch;}
.staff-sidebar{width:240px;background:#1a1a1a;color:#cfcfcf;padding:1.5rem 0;flex-shrink:0;}
.staff-sidebar .nav-label{font-size:.7rem;font-weight:800;letter-spacing:1.2px;color:#666;padding:0 1.5rem .6rem;text-transform:uppercase;}
.staff-sidebar a{display:flex;align-items:center;gap:.75rem;padding:.75rem 1.5rem;color:#cfcfcf;text-decoration:none;font-size:.9rem;font-weight:600;border-left:3px solid transparent;transition:background .15s,color .15s,border-color .15s;}
.staff-sidebar a:hover{background:#252525;color:#fff;}
.staff-sidebar a.active{background:#252525;color:#fff;border-left-color:var(--sk-red);}
.staff-sidebar a i{font-size:1.05rem;width:18px;text-align:center;}
.staff-main{flex:1;padding:2rem;min-width:0;position:relative;}
@media (max-width:768px){
  .staff-shell{flex-direction:column;}
  .staff-sidebar{width:100%;padding:.5rem 0;}
  .staff-sidebar a{padding:.6rem 1rem;}
}
</style>
</head>
<body>
