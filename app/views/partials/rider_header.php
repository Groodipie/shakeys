...
$nav = [
  'dashboard' => ['label' => 'Dashboard', 'icon' => 'bi-speedometer2', 'href' => '/rider/dashboard'],
  'orders'    => ['label' => 'Assigned Orders', 'icon' => 'bi-receipt', 'href' => '/rider/orders'],
];
?>
<!DOCTYPE html>
<html lang="en">
...
<title><?= e($pageTitle ?? "Rider — Shakey's") ?></title>
...
.rider-shell{flex:1;display:flex;align-items:stretch;}
.rider-sidebar{width:240px;background:#1a1a1a;color:#cfcfcf;padding:1.5rem 0;flex-shrink:0;}
.rider-sidebar .nav-label{font-size:.7rem;font-weight:800;letter-spacing:1.2px;color:#666;padding:0 1.5rem .6rem;text-transform:uppercase;}
.rider-sidebar a{display:flex;align-items:center;gap:.75rem;padding:.75rem 1.5rem;color:#cfcfcf;text-decoration:none;font-size:.9rem;font-weight:600;border-left:3px solid transparent;transition:background .15s,color .15s,border-color .15s;}
.rider-sidebar a:hover{background:#252525;color:#fff;}
.rider-sidebar a.active{background:#252525;color:#fff;border-left-color:var(--sk-red);}
.rider-sidebar a i{font-size:1.05rem;width:18px;text-align:center;}
.rider-main{flex:1;padding:2rem;min-width:0;position:relative;}
@media (max-width:768px){
  .rider-shell{flex-direction:column;}
  .rider-sidebar{width:100%;padding:.5rem 0;}
  .rider-sidebar a{padding:.6rem 1rem;}
}
</style>
</head>
<body>
...
<div class="logo-bar">
  ...
  <div class="dropdown">
    <button class="avatar-btn" type="button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Account menu">
      <?= e(strtoupper($riderName[0] ?? 'R')) ?>
    </button>
    <ul class="dropdown-menu dropdown-menu-end avatar-menu">
      <li class="menu-header">
        <div class="label">Signed in as</div>
        <div class="name"><?= e($riderName) ?></div>
      </li>
      <li>
        <a class="dropdown-item" href="<?= e(url('/rider/logout')) ?>">
