...
// Fetch recommended products (first 4 available)
$recommended = $pdo->query(
    "SELECT * FROM Product WHERE Prod_Status='Available' ORDER BY Prod_ID LIMIT 4"
)->fetchAll();

// Fetch current promotions (valid today)
$today = date('Y-m-d');
$promos = $pdo->prepare(
    "SELECT * FROM Promotion WHERE Promo_ValidFrom <= ? AND Promo_ValidTo >= ? LIMIT 3"
);
$promos->execute([$today, $today]);
$activePromos = $promos->fetchAll();

// Fetch pizza products
$pizzas = $pdo->query(
    "SELECT * FROM Product WHERE Prod_Type='Pizza' AND Prod_Status='Available' LIMIT 6"
)->fetchAll();
      ...
      <div class="carousel-item <?= $i===0?'active':'' ?>">
        <div class="hero-banner d-flex align-items-center justify-content-between" style="min-height:220px;">
          <div style="flex:1;">
            ...
            <p class="text-secondary mb-3" style="font-size:.9rem;"><?= htmlspecialchars($p['Promo_Description']) ?></p>
  ...
  <div class="row g-3 mb-4">
    ...
    <div class="col-6 col-md-3">
      <div class="food-card">
        ...
        <div class="p-3">
          <h6 class="fw-bold mb-1" style="font-size:.9rem;"><?= htmlspecialchars($prod['Prod_Name']) ?></h6>
          <p class="text-muted mb-2" style="font-size:.78rem;"><?= htmlspecialchars($prod['Prod_Category']) ?></p>
          <div class="d-flex align-items-center justify-content-between">
            <span class="price">₱<?= number_format($prod['Prod_BasePrice'],2) ?></span>
            <form method="POST" action="add_to_cart.php">
              <input type="hidden" name="prod_id"    value="<?= $prod['Prod_ID'] ?>">
              <input type="hidden" name="prod_name"  value="<?= htmlspecialchars($prod['Prod_Name']) ?>">
  ...
  <div class="row g-3 mb-5">
    ...
    <div class="col-6 col-md-4 col-lg-2">
      <div class="food-card">
        <div class="thumb">🍕</div>
        <div class="p-3">
          <h6 class="fw-bold mb-1" style="font-size:.85rem;"><?= htmlspecialchars($prod['Prod_Name']) ?></h6>
