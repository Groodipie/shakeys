...
<div class="container-fluid px-3 px-md-4 py-3">
  ...
  <form method="GET" class="mb-4">
    <div class="input-group" style="max-width:400px;">
      <input type="text" name="search" class="form-control" placeholder="Search menu items..." value="<?= htmlspecialchars($search) ?>">
    ...
    <div class="col-6 col-md-3 col-lg-2">
      <a href="menu.php?category=<?= urlencode($cat['Prod_Category']) ?>" class="text-decoration-none">
        <div class="text-center p-3 rounded-3 border <?= $active ? 'border-danger' : 'border-light bg-white' ?>" style="cursor:pointer;">
          ...
          <div class="fw-bold mt-1" style="color:var(--sk-red);font-size:.8rem;"><?= htmlspecialchars($cat['Prod_Category']) ?></div>
        </div>
      </a>
    </div>
    <?php endforeach; ?>
  ...
  <h5 class="section-title mb-3">
    <?= $category ? htmlspecialchars($category) : ($search ? "Results for \"$search\"" : 'All Menu Items') ?>
  ...
  <div class="row g-3 mb-5">
    ...
    <div class="col-6 col-md-4 col-lg-3">
      <div class="food-card">
        ...
        <div class="p-3">
          <span class="badge mb-2" style="background:#fdf0f0;color:var(--sk-red);font-size:.72rem;"><?= htmlspecialchars($prod['Prod_Category']) ?></span>
          <h6 class="fw-bold mb-1" style="font-size:.9rem;"><?= htmlspecialchars($prod['Prod_Name']) ?></h6>
          <p class="text-muted mb-2" style="font-size:.78rem;"><?= htmlspecialchars($prod['Prod_Type']) ?></p>
